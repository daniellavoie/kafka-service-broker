...
public class KafkaJunitExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource {
	...
	private static void startUpZookeeper() throws IOException, InterruptedException {
		...
		zooKeeperServer = new ZooKeeperServer(zkSnapshostsDir, zkLogsDir, 2000);
	...
	private static void startUpKafka() {
		...
		kafkaProperties.setProperty("super.users",
				"User:client;User:admin;User:broker;User:connect;User:servicebroker");
		...
		kafkaProperties.setProperty("listener.name.sasl_plaintext.plain.sasl.jaas.config",
				"org.apache.kafka.common.security.plain.PlainLoginModule required " + "    username=\"admin\" "
						+ "    password=\"admin-secret\" " + "    user_broker=\"broker-secret\" "
						+ "    user_client=\"client-secret\" " + "    user_servicebroker=\"servicebroker-secret\" "
